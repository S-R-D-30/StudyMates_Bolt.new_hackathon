/*
  ...
  3. Triggers
    - Creates triggers for various automated actions
*/
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_follower_counts_trigger') THEN
...
EXCEPTION
  WHEN others THEN
    -- If there's an error, it might be because the relation doesn't exist yet
    NULL;
END
$$;
...
-- Trigger to add creator as community member
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'add_creator_as_member_trigger') THEN
...
-- Trigger to add host as session participant
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'add_host_as_participant_trigger') THEN
...
-- Trigger for updating infovid likes count
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_infovid_likes_count_trigger') THEN
...
-- Trigger for updating course enrollments
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_course_enrollments_trigger') THEN
...
-- Trigger for updating chat last activity
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_chat_last_activity_trigger') THEN
...
-- Trigger for updating note saves count
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_note_saves_count_trigger') THEN
...
-- Trigger for updating flip card set saves count
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_flip_card_set_saves_count_trigger') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'notes' AND policyname = 'Users can read own notes') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'flip_card_sets' AND policyname = 'Public flip card sets are viewable by everyone') THEN
    CREATE POLICY "Public flip card sets are viewable by everyone" ON flip_card_sets
      FOR SELECT USING (is_public = true);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'flip_card_sets' AND policyname = 'Users can insert own flip card sets') THEN
    CREATE POLICY "Users can insert own flip card sets" ON flip_card_sets
      FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'flip_card_sets' AND policyname = 'Users can read own flip card sets') THEN
    CREATE POLICY "Users can read own flip card sets" ON flip_card_sets
      FOR SELECT USING (auth.uid() = user_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'flip_card_sets' AND policyname = 'Users can update own flip card sets') THEN
    CREATE POLICY "Users can update own flip card sets" ON flip_card_sets
      FOR UPDATE USING (auth.uid() = user_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'flip_card_sets' AND policyname = 'Users can delete own flip card sets') THEN
